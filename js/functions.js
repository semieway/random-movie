...
export function renderMovie(options = null) {
  getMovieInfo(options).then(data => {
    ...
    let $countryFlags = document.querySelector('.countries');
    ...
    let $cast = document.querySelector('.cast-list');
    $cast.innerHTML = '';
    ...
    let $youtubeTrailer = document.querySelector('.trailer');
    if (data.trailer) {
    ...
    if ($cast.scrollWidth <= 768) {
      document.querySelector('.cast-scroll-button').classList.add('scrolled');
    }
...
export async function getMovieInfo(options = null) {
...
export async function rollMovie(options = null) {
...
/* IMDb rating circle */
export function showImdbRating(rating = 0) {
...
export function showMetacriticRating(rating = 0) {
